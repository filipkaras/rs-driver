...
struct ChallengeView: View {
    ...
    @StateObject var pickupViewModel = PickupViewModel(listType: "challenges")
    @StateObject var pagingViewModel = PagingViewModel<PickupViewModel>(source: PickupViewModel(listType: "challenges"))
    ...
    var body: some View {
                ...
                if pagingViewModel.items.count > 0 {
                    ...
                    VStack {
                        Image(systemName: "list.bullet.rectangle.portrait")
                            .font(.system(size: 60))
                            .padding(.top, 60)
                        Text("Aktuálne nie sú dostupné žiadne výzvy.")
        ...
        .onChange(of: notificationManager.refreshChallenge) { newValue in
            if notificationManager.refreshChallenge == false { return }
            notificationManager.refreshChallenge = false
            pagingViewModel.reloadData()
        }
    }
}
...
struct ChallengeListView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        ChallengeView(pagingViewModel: pagingViewModel)
        ChallengeView(pagingViewModel: pagingViewModel).preferredColorScheme(.dark)
    }
}
